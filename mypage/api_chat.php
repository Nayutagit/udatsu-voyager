...
/**
 * api_chat.php
 * ボイスジャーナリング(VJ)を解析し、ログインユーザーのAI分身としてチャット応答を生成するAPI。
...
foreach ($validPosts as $p) {
    ...
    $title = $p['title'] ?? '無題';
    $date = $p['date'] ?? '';
    $category = $p['category'] ?? 'ボイスジャーナリング';
    if ($category === 'ボイスジャーナル') {
        $category = 'ボイスジャーナリング';
    }
    // 整文を優先
...
// システムインストラクションの作成
$botStyleReference = $profile['bot_style_reference'] ?? '';
$stylePromptAddendum = '';
if (!empty($botStyleReference)) {
    $stylePromptAddendum = "\n7. **話し方の特別な指示・手本（最優先）**: ユーザーから以下の「話し方の手本テキスト・指示」が設定されています。回答の口調、語尾、文字使い、および言葉遣いは、必ずこの手本のスタイルを厳密に模倣・再現してください：\n【話し方の手本テキスト・指示】\n{$botStyleReference}\n";
}
...
2. **語り口調・トーン**: 丁寧で自然な日本語（です・ます調を基本とし、馴れ馴れしすぎない、礼儀正しく落ち着いたトーン）で回答してください。過度にフレンドリーになったり、タメ口や馴れ馴れしい表現は避けてください。一人称は「私」または「僕」としてください。
3. **知識ソース**: 下記に提供する「ボイスジャーナリング(VJ)ログの蓄積」をあなたの唯一無二の知識ベースとしてください。回答の中で「何月何日のジャーナルで話したように」のような日付や時期を明示する前置きは、返答から省略してください。代わりに、回答に関連するジャーナルがある場合、返答テキスト（reply）の最後に、以下のフォーマットで関連リンクを配置してください：
関連：[ジャーナルのタイトル](view_post.php?uid={$uid}&id=ジャーナルID)
※関連ジャーナルが複数ある場合は改行して並べてください。リンク以外の文脈では、わざわざ日付や時期の引用は言及しなくて結構です。
