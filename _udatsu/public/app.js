...
function renderSchedule() {
  ...
  list.innerHTML=visible.map(s=>{
    ...
    return `<article class="schedule-row"><div class="schedule-date">${date(s.start)}<small>${time(s.start)}–${time(s.end)}</small></div><div class="schedule-info"><span class="badge">${c?'開催確定・'+(s.remaining>0?'あと'+s.remaining+'席':'満席／手続き中'):'テーマを選べます'}</span><strong>${escape(c?.title||'あなたの受けたい講座で開催できます')}</strong><p>${escape(s.format)} / ${escape(s.location)}</p></div><button class="button small" data-slot="${s.id}" ${c&&s.remaining===0?'disabled':''}>${c?'この講座に参加する':'講座を選ぶ'} ↗</button></article>`;
  }).join('');
  list.querySelectorAll('[data-slot]').forEach(b=>b.onclick=()=>{
    ...
    if(slot.courseId){currentCourse=data.courses.find(c=>c.id===slot.courseId);confirmBooking(slot);}
      ...
      open(`<h2 id="dialog-title">${date(slot.start)} ${time(slot.start)}<br>何を学びますか？</h2><p>最初の方のお支払いで、この時間の講座が決まります。</p><div class="slot-options">${data.courses.filter(c=>Date.parse(slot.end)-Date.parse(slot.start)>=c.minutes*60000).map(c=>`<button class="slot-option" data-pick="${c.id}"><span>${escape(c.title)}</span><small>${c.minutes}分 / ${yen(c.price)}${c.kind==='private'?' / 1対1':''}</small></button>`).join('')}</div>`);
...
async function showCourse(courseId, preselected) {
  ...
  let available=slots.filter(s=>(s.status==='available'||(s.courseId===c.id&&s.remaining>0))&&Date.parse(s.end)-Date.parse(s.start)>=c.minutes*60000);
